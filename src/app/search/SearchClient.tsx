...
import { Filter, Search as SearchIcon, MapPin, SlidersHorizontal, Database } from "lucide-react";
...
export default function SearchClient({ 
  ...
  const filteredRooms = initialListings.filter((room) => {
    if (filterType !== "All" && room.property_type !== filterType) return false;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {sortedRooms.map((room) => (
              <RoomCard 
                key={room.id} 
                room={room} 
                initialFavorite={initialFavorites.includes(room.id)}
              />
            ))}
          </div>
        )}
