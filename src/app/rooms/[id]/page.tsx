...
export default function RoomDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!room) {
    return <div className="p-20 text-center font-headline text-2xl">Room not found</div>;
  }
      ...
      <div className="container px-4 mx-auto py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 grid-rows-2 gap-3 h-[300px] md:h-[500px] mb-8 rounded-2xl overflow-hidden shadow-lg">
          ...
          <div className="relative hidden md:block">
            <Image src={room.photos[1] || room.photos[0]} alt={room.title} fill className="object-cover" />
          </div>
          <div className="relative hidden md:block">
            <Image src={room.photos[0]} alt={room.title} fill className="object-cover brightness-75" />
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-6 rounded-2xl shadow-sm">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                 <div className="flex flex-col items-center gap-2 p-4 rounded-xl bg-primary/5 border border-primary/10">
                    <Wifi className={`h-8 w-8 ${room.wifiAvailable ? 'text-primary' : 'text-muted/30'}`} />
                 ...
                 <div className="flex flex-col items-center gap-2 p-4 rounded-xl bg-secondary/5 border border-secondary/10">
                    <Wind className={`h-8 w-8 ${room.acAvailable ? 'text-secondary' : 'text-muted/30'}`} />
                 ...
                 <div className="flex flex-col items-center gap-2 p-4 rounded-xl bg-yellow-50 border border-yellow-100">
                    <Zap className={`h-8 w-8 ${room.inverterAvailable ? 'text-yellow-600' : 'text-muted/30'}`} />
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm">
               ...
               <p className="text-muted-foreground mb-6">
                 This property is located in a prime spot. {room.nearestCommunication}. Perfect for commuters and students.
               </p>
               
               <div className="bg-green-50 border border-green-100 rounded-xl p-4 flex items-center gap-3">
                 <ShieldCheck className="h-6 w-6 text-primary shrink-0" />
                 <div>
                   <p className="font-bold text-sm">Verified Landlord</p>
                   <p className="text-xs text-muted-foreground">This landlord has provided valid contact and identity details.</p>
                 </div>
               </div>
            </div>
          </div>
